...
void forget_map(int chance_forgotten, bool force)
{
    ...
    const bool rotting_map = (you.level_type == LEVEL_LABYRINTH || you.level_type == LEVEL_ABYSS);
    const bool rot_resist = (you.level_type == LEVEL_LABYRINTH && you.species == SP_MINOTAUR 
                     || you.level_type == LEVEL_ABYSS && you.religion == GOD_LUGONU) ? true : false; 
    ...
    for (rectangle_iterator ri(0); ri; ++ri)
    {
        if (!you.see_cell(*ri))
        {
            ...
            if (!doDecay) 
            {
                if (rotting_map)
                {
                    int chance = pow(geometric_chance, std::max(1,dist-radius)) * scalar;  
